...
//        printf("Parent\n");
    ...
    EmcController::loadState EmcController::Init()
    {
        ...
        if(state == lsUnloaded || state == lsFault )
        {
            ...
            char buff[256];
            std::ifstream hal( std::string(m_emcPath + std::string("configs/miniemc.hal")).c_str() );
            m_used_map.clear();
            while(hal.good() )
            {
    ...
    {
        std::string cmd = m_emcPath + "scripts/emc_start";
        std::string arg = m_emcPath + "configs/miniemc.ini";
        int run = ShellCmd(cmd.c_str() , arg.c_str());

        if(run < 0)
        {
            // EMC2 is not started due to some errors
           AddErrorString (elError, "Unable to load EMC core, call support!!!");
           cmd = m_emcPath + "scripts/emc_stop";
           ::system(cmd.c_str());
           std::cout << "Unable to load EMC core, call support!!!" << std::endl;
           return false;
        }
        ::initMain();
        ::iniLoad( arg.c_str() );
    ...
    void EmcController::Exit_internal()
    {
        ...
        std::string cmd = m_emcPath + "scripts/emc_stop";
//        std::cout << "exec cmd " << cmd << std::endl;
        //::system(cmd.c_str());
        ShellCmd(cmd.c_str(), NULL);
 //       printf("Exit() leave");
    }
    ...
    bool EmcController::SetControllerMode(eCtlMode mode)
    {
        ...
        if( GetControllerMode() != mode)
        {
            ...
            {
                AddErrorString( elWarning , "Mashine is busy!" );
                rc = false;
            }
        }
        return rc;
    ...
    void EmcController::MashineOn()
    {
        Coord cord; // Zero offset
        if(GetInitState() != lsLoaded)
            return;
        sendMachineOn();
 //       std::cerr << "MashineON() call" << std::endl;
    }

    void EmcController::MashineOff()
    {
        if(GetInitState() != lsLoaded)
            return;
        sendMachineOff();
//        std::cerr << "MashineOFF() call" << std::endl;
    }

    Coord EmcController::GetCurrentOrigin()
    {
        Coord cord;
        cord.SetPosByIndex(0, emcStatus->task.origin.tran.x );
        cord.SetPosByIndex(1, emcStatus->task.origin.tran.y );
        cord.SetPosByIndex(2, emcStatus->task.origin.tran.z );
        cord.SetPosByIndex(3, emcStatus->task.origin.a );
        cord.SetPosByIndex(4, emcStatus->task.origin.b );
        cord.SetPosByIndex(5, emcStatus->task.origin.c );
        return cord;
    }
...
//        std::cerr << "SendJogCont(" << axis << "," << speed << ")=" << rc << std::endl;
        }
        return rc;
    }
...
//        std::cerr << "SendJogStop(" << axis << ")=" << rc << std::endl;
        return rc;
    }
...
//        std::cerr << "sendFeedOverride(" << ovr << ")" << std::endl;
    }

    bool EmcController::IsMashineFree()
    {
        if(GetInitState() != lsLoaded)
            return false;
         ...
         {
            AddErrorString(elWarning, "Mashine is OFF!");
            return false;
         }
         if(!IsInpos())
         {
            AddErrorString(elWarning, "Mashine is moving!");
            return false;
         }
         return  true;
    }
     ...
     Coord EmcController::GetCurrentOffset()
     {
        ...
        if(GetInitState() != lsLoaded)
         ...
         offset.SetPosByIndex(0, emcStatus->task.origin.tran.x);
         offset.SetPosByIndex(1, emcStatus->task.origin.tran.y);
         offset.SetPosByIndex(2, emcStatus->task.origin.tran.z);
         offset.SetPosByIndex(3, emcStatus->task.origin.a);
         offset.SetPosByIndex(4, emcStatus->task.origin.b);
         offset.SetPosByIndex(5, emcStatus->task.origin.c);
         return offset;
     }
...
//            std::cerr << "OverrideLimit is " << val << std::endl;
        }
     }
    ...
    ListPrt EmcController::GetActualProgramContext()
    {
        ...
        if( GetLastFileName() != "" )
        {
            ...
            {
     //           std::cout << __FUNCTION__ << ": update content, idx=" << m_last_context_index << " newIdx=" << GetLineNumber() / GetNumLinesPerContext() << std::endl;
                /*
    ...
    void EmcController::RunProgram(int line)
    {
        ...
        if( IsMashineFree() && GetInterpState() == eiIdle)
        {
            SetControllerMode(emAuto);
//            std::cout << "Trying to run program " << GetLastFileName() << std::endl;
            ::sendProgramRun(line);
        }
    }
