...
static const char* emcCFGJson = "/tmp/emc_config.json";
...
std::string GetDriectoryListHtml(const char* dir)
{
    std::stringstream rc;
    std::string root="/media/";
...
bool ReqHandler::operator () (mongoose::event_type type, mongoose::tcp_connection connection, mongoose::web_request request)
{
    ...
    {
       // std::clog << request.get_log_message() << std::endl;
    }
...
void DoCmd()
{
    ...
    while( GetCmdQueueSize() != 0 )
    {
        ...
        } else if( cmd.first == "jog")
        {
            ...
            if( dist != 0 )
            {
                //std::cerr << "Starting jogging for axis " << axis << " to pos. " << dist << " at speed " <<  pCfg->GetMaxVelocity() << std::endl;
                pCtrl->SendJogIncr(pCtrl->AxisToIndex(axis), 1000.0*60.0 , dist);
            } else
            {
                LastJoggingAxis = pCtrl->AxisToIndex(axis);
                //std::cerr << "Continus joggign at " << cmd.second[1] << std::endl;
        ...
        {
            std::string file = "/media/" + cmd.second;
